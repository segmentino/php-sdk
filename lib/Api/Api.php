...
use Segmentino\QueryBuilder\QueryBuilder;
use Segmentino\Auth\ApiAuth;
use Segmentino\Auth\AuthInterface;
...
class Api implements LoggerAwareInterface
{
    ...
    /**
     * Name of the array element where the property is
     *
     * @var string
     */
    ...
    /**
     * Array of deprecated endpoints to use if a response fails as a 404 with a previous version of Segmentino
     *
     * @var array
     */
        ...
        if (!$bcEndpoint || !$this->bcTesting || ($bcEndpoint && $this->bcTesting && $this->bcAttempt)) {
            ...
            if (strpos($url, 'http') === false) {
                $error = array(
                    'code'    => 500,
                    'message' => sprintf(
                        'URL is incomplete.  Please use %s, set the base URL as the third argument to $SegmentinoApi->newApi(), or make $endpoint a complete URL.',
                ...
                try {
                    ...
                    $this->getLogger()->error('Failed connecting to Segmentino API: '.$e->getMessage(), array('trace' => $e->getTraceAsString()));
            ...
            if (!empty($error)) {
                ...
                $this->getLogger()->error('Segmentino API returned errors: '.var_export($response['errors'], true));
            }
    ...
    /**
     * Returns Segmentino version from the HTTP response headers
     *
     * @return string|null if not known
     */
    public function getSegmentinoVersion()
    {
        $headers = $this->auth->getResponseHeaders();

        if (isset($headers['Segmentino-Version'])) {
            return $headers['Segmentino-Version'];
        }
